...
BuildScreenTable:	xor		si,si	; even lines
...
.vbplot:		mov		di,[BackBufferTable+si]	; lookup start ver
				mov		dl,[es:di+bx]
				or		dl,dh
				mov		[es:di+bx],dl
...
.vfplot:		mov		di,[BackBufferTable+si]	; lookup start ver
				mov		dl,[es:di+bx]
				or		dl,dh
				mov		[es:di+bx],dl
...
;	EOR		#255
;	TAY
GetSin000090: ; if (x <  90) / (X=0) return  sine_table[   y]
;	LDA		SINTAB,Y
;	TAY
;	RTS
;SIN_COS_270_000: ; if (X=3) return -sine_table[90-y]
