...
;	call	Hex8
	...
	mov		bp,[testx]
	mov		cx,[testy]
	call	Perspective16	; in = bl=z/ah=v/dx=h/dh=hsign - out = bl=v/ax=h
	mov		bp,dx
	xor		bh,bh
	call	Plot11
...
; in = bl=z/bh=v/dx=h/dh=hsign - out = bl=v/ax=h ???? possible

Perspective8before: ; in = si=z/bl=v/bh=h/?=hsign - out = bl=v/ax=h
...
Perspective8Test: ; in = bx=z (-256+256) ax=v (-256+256) dx=h/dh=hsign - out = bl=v/dx=h
...
Perspective16: ; in = bx=z/cx=v/bp=h/dh=hsign - out = bl=v/dx=h
	shr		bx,1
	mov		al,[persvtab8+bx]
	shr		cx,1
	MUL		cl
...
Perspective16bk: ; in = bx=z/cx=v/dx=h/dh=hsign - out = bl=v/dx=h
