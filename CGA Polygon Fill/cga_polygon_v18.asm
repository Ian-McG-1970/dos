...
%macro left_right_same 0
	...
	and		esi,[es:di]				; get screen												; ????????
;	and		ebx,esi					; and it with (gap) to create what needs to go on screen	; ??    ??
	or		esi,ecx					; or it with (middle)										;
	mov		[es:di],esi				; put screen
%endmacro

%macro left_right_same_bk 0
	shl		bx,2					; start hor *4
	mov		ecx,[FillLeftTab32+bx]	; get left edge
	shl		bp,2					; start hor *4
	and		ecx,[FillRightTab32+bp]	; and with right edge to create (middle)					; XX    XX
	...
	and		ecx,eax					; and with colour
	not		esi						; not it to create (gap)									;   XXXX  
	mov		ebx,[es:di]				; get screen												; ????????
	and		ebx,esi					; and it with (gap) to create what needs to go on screen	; ??    ??
	or		ebx,ecx					; or it with (middle)										;
	mov		[es:di],ebx				; put screen
%endmacro

%macro left_edge 0
	shl		bx,2			; start hor *4
	mov		esi,[es:di]					; get screen								(ecx) = es:di							mov	ecx,[es:di]
	and		esi,[FillLeftTab32V2+bx]	; and with gap (left/right)					(ecx) = screen that is still needed		and ecx,[leftgaptab+bx]
	mov		ebx,[FillLeftTab32+bx]		; get with not_gap (left/right)				(esi) = colour that is still needed		and esi,[leftnongaptab+bx]
	and		ebx,eax						; and colour								(esi) = eax								mov	esi,eax
	or		ebx,esi						; or gap and not_gap						(ecx) = final result					or	ecx,esi
	mov		[es:di],ebx					; put on screen								(ecx) = es:di							mov [es:di],ecx
%endmacro
...
.next			add		si,4
			...
			mov		dl,bh
			mov		al,[Colour]		; get colour
			xor		bh,bh
			
			test	bl,1
			jz		.col_cont
				ror 	al,2		; odd so move stipple across
.col_cont:	mov 	ah,al
			shl 	eax,8
			mov 	al,ah
			shl 	eax,8
