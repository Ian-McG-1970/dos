...
.key_esc:	mov	al,1
		mov	[Exit],al
		ret
.key_1:	mov ax,[XOffset]
		sub ax,37
		mov [XOffset],ax
		ret
.key_2:	mov ax,[XOffset]
		add ax,37
		mov [XOffset],ax
		ret
.key_3:	mov ax,[YOffset]
		sub ax,37
		mov [YOffset],ax
		ret
.key_4:	mov ax,[YOffset]
		add ax,37
		mov [YOffset],ax
		ret
.key_5:	mov ax,[ZOffset]
		sub ax,37
		mov [ZOffset],ax
		ret
.key_6:	mov ax,[ZOffset]
		add ax,37
		mov [ZOffset],ax
		ret
.key_q:	mov	ax,[XAngle]
...
Perspective:	inc		ch	; in - cx=z ax=x bp=y out - ax/bp	; z +256
...
; rotateobject
; get object 16bit x/y/z
; subtract 16bit position x/y/z
; if diffx/y/z is greater than value
; rotate offset
; classify point
; if onscreen plot it
...
.rotate_point_loop:	push	si
	...
	mov	[Test5],cx
					...
					test	cx,cx			; might not be needed?
					js		.point_behind


.point_in_front:		call	Perspective				; in - ax =h /bp =v /cx =z / ax=h  bp=v
	mov	[Test6],bp
	mov	[Test7],ax
						call	ClassifyPoint			; in = ax=h / bp=v
	mov	[Test10],dx
						test	dl,dl					; might not be needed?
						jne		.point_behind
							mov		bx,ax				; h
							xchg	bx,bp				; hor
